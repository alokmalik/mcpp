...
class BFSengine:
    def __init__(self,l,b,graph) -> None:
        self.l=l
        self.b=b
        self.neighbours=[]
        self.second_degree=[99,2929,2999,3564,6464,7029,9929,9999] 
    ...
    def graph_constructor(self,crds,agent,graph,global_graph,depth):
        source=crds[agent]
        ...
        while queue:
            node=queue.pop(0)
            for neighbour in global_graph[node]:
        ...
        for node in nodes:
            if node not in graph:
                graph.add_node(node,explored=0)
                for neighbour in global_graph[node]:
                    if neighbour in graph:
                        graph.add_edge(node,neighbour)
        return graph

    def action_list(self,crds,agent,graph,global_graph,depth):
        l,b=self.l,self.b
        graph=self.graph_constructor(crds,agent,graph,global_graph,depth)
        source=crds[agent]
